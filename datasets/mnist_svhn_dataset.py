...
class MnistSvhnDataset(BaseDataset):
    def __init__(self, opt):
        ...
        self.mnist = MNIST(os.path.join(opt.dataroot, 'mnist'), train=opt.isTrain, download=True)
        #svhn_mat_extra = scipy.io.loadmat(os.path.join(opt.dataroot, 'svhn/extra_32x32.mat'))

        if opt.isTrain:
            svhn_mat_train = scipy.io.loadmat(os.path.join(opt.dataroot, 'svhn/train_32x32.mat'))
            svhn_np = np.array(svhn_mat_train['X'])
            self.svhn = np.transpose(svhn_np, (3, 0, 1, 2))
            self.svhn_label = np.array(svhn_mat_train['y'])
        else:
            svhn_mat_test = scipy.io.loadmat(os.path.join(opt.dataroot, 'svhn/test_32x32.mat'))
            svhn_np = np.array(svhn_mat_test['X'])
            self.svhn = np.transpose(svhn_np, (3, 0, 1, 2))
            self.svhn_label = np.array(svhn_mat_test['y'])
